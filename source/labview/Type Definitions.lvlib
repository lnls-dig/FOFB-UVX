<?xml version='1.0' encoding='UTF-8'?>
<Library LVVersion="11008008">
	<Property Name="NI.Lib.Icon" Type="Bin">%1#!#!!!!!)!"1!&amp;!!!-!%!!!@````]!!!!"!!%!!!(]!!!*Q(C=\&gt;7R=2MR%!81N=?"5X&lt;A91M&lt;/W-,&lt;'&amp;&lt;9+K1,7Q,&lt;)%N&lt;!NMA3X)DW?-RJ(JQ"I\%%Z,(@`BA#==ZB3RN;]28_,V7@P_W`:R`&gt;HV*SU_WE@\N_XF[3:^^TX\+2YP)D7K6;G-RV3P)R`ZS%=_]J'XP/5N&lt;XH,7V\SEJ?]Z#5P?=J4HP+5JTTFWS%0?=B$DD1G(R/.1==!IT.+D)`B':\B'2Z@9XC':XC':XBUC?%:HO%:HO&amp;R7QT0]!T0]!S0I4&lt;*&lt;)?=:XA-(]X40-X40-VDSGC?"GC4N9(&lt;)"D2,L;4ZGG?ZH%;T&gt;-]T&gt;-]T?.S.%`T.%`T.)^&lt;NF8J4@-YZ$S'C?)JHO)JHO)R&gt;"20]220]230[;*YCK=ASI2F=)1I.Z5/Z5PR&amp;)^@54T&amp;5TT&amp;5TQO&lt;5_INJ6Z;"[(H#&gt;ZEC&gt;ZEC&gt;Z$"(*ETT*ETT*9^B)HO2*HO2*(F.&amp;]C20]C2)GN4UE1:,.[:/+5A?0^NOS?UJ^3&lt;*\9B9GT@7JISVW7*NIFC&lt;)^:$D`5Q9TWE7)M@;V&amp;D,6;M29DVR]6#R],%GC47T9_/=@&gt;Z5V&gt;V57&gt;V5E&gt;V5(OV?^T[FTP?\`?YX7ZRP6\D=LH%_8S/U_E5R_-R$I&gt;$\0@\W/VW&lt;[_"&lt;Y[X&amp;],0^^+,]T_J&gt;`J@_B_]'_.T`$KO.@I"O[^NF!!!!!!</Property>
	<Property Name="NI.Lib.Version" Type="Str">1.0.0.0</Property>
	<Item Name="Type Acquisition Network Configuration.ctl" Type="VI" URL="../Types Definitions/Type Acquisition Network Configuration.ctl"/>
	<Item Name="Type Actuation Network Configuration.ctl" Type="VI" URL="../Types Definitions/Type Actuation Network Configuration.ctl"/>
	<Item Name="Type Actuation State.ctl" Type="VI" URL="../Types Definitions/Type Actuation State.ctl"/>
	<Item Name="Type Blocking Synchronization Objects.ctl" Type="VI" URL="../Types Definitions/Type Blocking Synchronization Objects.ctl"/>
	<Item Name="Type BPM and PS select cluster.ctl" Type="VI" URL="../Types Definitions/Type BPM and PS select cluster.ctl"/>
	<Item Name="Type BPM or PS Selector.ctl" Type="VI" URL="../Types Definitions/Type BPM or PS Selector.ctl"/>
	<Item Name="Type BPM Parameters.ctl" Type="VI" URL="../Types Definitions/Type BPM Parameters.ctl"/>
	<Item Name="Type BPM Variables.ctl" Type="VI" URL="../Types Definitions/Type BPM Variables.ctl"/>
	<Item Name="Type Configuration Change Acknowledge (Controller to GUI).ctl" Type="VI" URL="../Types Definitions/Type Configuration Change Acknowledge (Controller to GUI).ctl"/>
	<Item Name="Type Configuration Data (GUI to Controller).ctl" Type="VI" URL="../Types Definitions/Type Configuration Data (GUI to Controller).ctl"/>
	<Item Name="Type Configuration Loop Mode.ctl" Type="VI" URL="../Types Definitions/Type Configuration Loop Mode.ctl"/>
	<Item Name="Type Controller Operation Mode.ctl" Type="VI" URL="../Types Definitions/Type Controller Operation Mode.ctl"/>
	<Item Name="Type Controller Parameters.ctl" Type="VI" URL="../Types Definitions/Type Controller Parameters.ctl"/>
	<Item Name="Type Correction Algorithm Configuration.ctl" Type="VI" URL="../Types Definitions/Type Correction Algorithm Configuration.ctl"/>
	<Item Name="Type CPU Loads Array.ctl" Type="VI" URL="../Types Definitions/Type CPU Loads Array.ctl"/>
	<Item Name="Type CPU Loads.ctl" Type="VI" URL="../Types Definitions/Type CPU Loads.ctl"/>
	<Item Name="Type Data Server Command ID.ctl" Type="VI" URL="../Types Definitions/Type Data Server Command ID.ctl"/>
	<Item Name="Type Diagnostics Data.ctl" Type="VI" URL="../Types Definitions/Type Diagnostics Data.ctl"/>
	<Item Name="Type Diagnostics Sample.ctl" Type="VI" URL="../Types Definitions/Type Diagnostics Sample.ctl"/>
	<Item Name="Type FA Configuration.ctl" Type="VI" URL="../Types Definitions/Type FA Configuration.ctl"/>
	<Item Name="Type FA Data Source Selection.ctl" Type="VI" URL="../Types Definitions/Type FA Data Source Selection.ctl"/>
	<Item Name="Type FA File Parameters.ctl" Type="VI" URL="../Types Definitions/Type FA File Parameters.ctl"/>
	<Item Name="Type FA Request Parameters.ctl" Type="VI" URL="../Types Definitions/Type FA Request Parameters.ctl"/>
	<Item Name="Type FOFB Configuration.ctl" Type="VI" URL="../Types Definitions/Type FOFB Configuration.ctl"/>
	<Item Name="Type FOFB GUI.ctl" Type="VI" URL="../Types Definitions/Type FOFB GUI.ctl"/>
	<Item Name="Type FOFB Parameters.ctl" Type="VI" URL="../Types Definitions/Type FOFB Parameters.ctl"/>
	<Item Name="Type FOFB Variables.ctl" Type="VI" URL="../Types Definitions/Type FOFB Variables.ctl"/>
	<Item Name="Type GUI PS.ctl" Type="VI" URL="../Types Definitions/Type GUI PS.ctl"/>
	<Item Name="Type LNLS Control FOFB Configuration.ctl" Type="VI" URL="../Types Definitions/Type LNLS Control FOFB Configuration.ctl"/>
	<Item Name="Type LOCON_BIPOLAR_16BITS.ctl" Type="VI" URL="../Types Definitions/Type LOCON_BIPOLAR_16BITS.ctl"/>
	<Item Name="Type Monitoring Data Configuration.ctl" Type="VI" URL="../Types Definitions/Type Monitoring Data Configuration.ctl"/>
	<Item Name="Type Monitoring Data.ctl" Type="VI" URL="../Types Definitions/Type Monitoring Data.ctl"/>
	<Item Name="Type Monitoring Sample.ctl" Type="VI" URL="../Types Definitions/Type Monitoring Sample.ctl"/>
	<Item Name="Type Network Configuration.ctl" Type="VI" URL="../Types Definitions/Type Network Configuration.ctl"/>
	<Item Name="Type Non-blocking Synchronization Objects.ctl" Type="VI" URL="../Types Definitions/Type Non-blocking Synchronization Objects.ctl"/>
	<Item Name="Type PID Parameters.ctl" Type="VI" URL="../Types Definitions/Type PID Parameters.ctl"/>
	<Item Name="Type PS Parameters.ctl" Type="VI" URL="../Types Definitions/Type PS Parameters.ctl"/>
	<Item Name="Type PS Variables.ctl" Type="VI" URL="../Types Definitions/Type PS Variables.ctl"/>
	<Item Name="Type Response Matrix Measurement Configuration.ctl" Type="VI" URL="../Types Definitions/Type Response Matrix Measurement Configuration.ctl"/>
	<Item Name="Type Synchronization Objects.ctl" Type="VI" URL="../Types Definitions/Type Synchronization Objects.ctl"/>
	<Item Name="Type TCP State.ctl" Type="VI" URL="../Types Definitions/Type TCP State.ctl"/>
</Library>
